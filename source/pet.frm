...
Begin VB.Form frmPet 
   ...
   Begin VB.Frame fraPokemon 
      ...
      Begin VB.Label Label3 
         Caption         =   "Scroll to next or previous pokemon"
         Height          =   495
         Left            =   2520
         TabIndex        =   33
         Top             =   240
...
Private Sub mnuDTTF_Click()
  ...
  Print #2, , " "
  Print #2, , "Name:" & txtTrnName.Text
  Print #2, , "Trainer Class:" & cboTrnGrp.Text
  a = chkDuo.Value
  If a = 1 Then b = "Yes"
  If a = 0 Then b = "No"
  Print #2, , "Double Battle:" & b
  Print #2, , "Pokemon Held:" & txtPokemonHeld
  Print #2, , "Item 1:" & cboItem1.Text & "       " & "Item2:" & cboItem2.Text
  Print #2, , "Item 3:" & cboItem3.Text & "       " & "Item4:" & cboItem4.Text
  Print #2, , "            Pokemon"
  For i = 1 To txtPokemonHeld
    ...
    If Trainer(Editing).bGroup = &H17 Or Trainer(Editing).bGroup = &H18 Or Trainer(Editing).bGroup = &H1E Or Trainer(Editing).bGroup = &H5A Or Trainer(Editing).bGroup = &H54 Or Trainer(Editing).bGroup = &H19 Or Trainer(Editing).bGroup = &H57 Then
      ...
      Print #2, , "Level:" & txtLevel & " " & "Pokemon:" & cboSpecies.Text
      Print #2, , "Attack 1:" & cboAttack1.Text & "       " & "Attack2:" & cboAttack2.Text
      Print #2, , "Attack 3:" & cboAttack3.Text & "       " & "Attack4:" & cboAttack4.Text
      Print #2, , " "
    Else
      Print #2, , "Pokemon in slot #" & i
      Print #2, , ""
...
Dim a As String
a = InputBox("Please enter the new Trainer Class no more then 12 chars please")
Group(cboTrnGrp.ListIndex).Class = NameAsc2Sap(a)
...
'  End With
  ...
  Dim MyCDL As New cCommonDialog
  Dim TheFile As String
  If Not MyCDL.VBGetOpenFileName(TheFile, , , , , , "PET datafiles|*.pet") Then Exit Sub
...
Private Sub cmdPlusOne_Click()
  If Trainer(Editing).bSprite = &H93 Then cmdPlusOne.Enabled = False
  If Trainer(Editing).bSprite = &H92 Then cmdPlusOne.Enabled = True
  Trainer(Editing).bSprite = Trainer(Editing).bSprite + 1
  TrainerPics
End Sub
...
Private Sub cmdPrev_Click()
  If pokemon(hsbPokemon).iSpecies = &H0 Then cmdPrev.Enabled = False
  If pokemon(hsbPokemon).iSpecies = &H0 Then cmdPrev.Enabled = True
  If Gpokemon(hsbPokemon).iSpecies = &H0 Then cmdPrev.Enabled = False
  If Gpokemon(hsbPokemon).iSpecies = &H0 Then cmdPrev.Enabled = True
  If Trainer(Editing).bGroup = &H17 Or Trainer(Editing).bGroup = &H18 Or Trainer(Editing).bGroup = &H1E Or Trainer(Editing).bGroup = &H5A Or Trainer(Editing).bGroup = &H54 Then
    Gpokemon(hsbPokemon.Value).iSpecies = Gpokemon(hsbPokemon.Value).iSpecies - 1
    gpokemonPics
  ElseIf Trainer(Editing).bGroup = &H57 Then
    Epokemon(hsbPokemon.Value).iSpecies = Epokemon(hsbPokemon.Value).iSpecies - 1
    epokemonPics
  Else
...
txtMusic = Trainer(Editing).bSprite
...
cboAttack4.Clear
'MsgBox ("Rom closed")
'KAWA --- Opening a rom will trigger all this, including the message.
'         It looks very weird to see "Rom closed" while you haven't opened anything yet.
End Sub
...
'End With
  Dim MyCDL As New cCommonDialog
  Dim TheFile As String
  If Not MyCDL.VBGetSaveFileName(TheFile, , , "PET datafiels|*.pet") Then Exit Sub
...
mnuClrm_Click 'make sure it's clear
...
txtTheFile = TheFile
frmPet.Caption = "PET - " & Roms(RomData).Name
...
Trainer(Editing).bMusic = txtMusic
End Sub
...
picTrainer = LoadPicture(PICSDIR & "/trainer-" & Right("000" & Hex(Trainer(Editing).bSprite), 3) & ".bmp")
End Function
Private Function gpokemonPics()
If Gpokemon(hsbPokemon).iSpecies = &HFFFF Then
picPokemon = LoadPicture(PICSDIR & "/pkmn-000.bmp")
ElseIf Gpokemon(hsbPokemon).iSpecies = &HFF24 Then
picPokemon = LoadPicture(PICSDIR & "/pkmn-000.bmp")
Else
picPokemon = LoadPicture(PICSDIR & "/pkmn-" & Right("000" & Hex(Gpokemon(hsbPokemon).iSpecies), 3) & ".bmp")
End If
End Function
...
End If
End Function
Private Function epokemonPics()
If Epokemon(hsbPokemon).iSpecies = &HFFFF Then
picPokemon = LoadPicture(PICSDIR & "/pkmn-000.bmp")
Else
picPokemon = LoadPicture(PICSDIR & "/pkmn-" & Right("000" & Hex(Epokemon(hsbPokemon).iSpecies), 3) & ".bmp")
End If
End Function
