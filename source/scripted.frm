...
Private Sub cmdCompile_Click()
  ...
  LastROM = NewFile
  INIWrite "EliteMap", "Rubikon", "Last ROM", NewFile
End Sub
...
Private Sub cmdLoad_Click()
  ...
  Dim WarnedForTrainerBattle As Boolean
  ...
  If InStr(LCase(txtFile), ".gba:") Or InStr(LCase(txtFile), ".bin:") Then
    ...
    TehOffset = Val("&H" & Mid(txtFile, InStr(LCase(txtFile), ".") + 5))
  End If
  ...
  Do While Ryan < numsnips
    ...
    offset = snips(Ryan) - IIf(snips(Ryan) > &H8000000, &H8000000, 0)
    break = False
    If offset <= 0 Then Exit Do
    txtCode = txtCode & "#org 0x" & Hex(offset) & vbCrLf
    Seek #1, offset + 1
    Do
      If offset <= 0 Then Exit Do
      'Seek #1, offset
      ...
      ElseIf wite = 6 Then 'jumping version
        ...
        If notinsnips(wow2 - IIf(wow2 > &H8000000, &H8000000, 0) + 1) = True Then
          snips(numsnips) = wow2 - IIf(wow2 > &H8000000, &H8000000, 0)
          numsnips = numsnips + 1
        End If
      ...
      ElseIf wite = &HF Then
        ...
        Get #1, , wow2
        If wite2 Then
          txtCode = txtCode & "loadptr 0x" & Hex(wite2) & " 0x" & Hex(wow2)
        Else
          ...
          End If
        End If
      ElseIf wite = &H4F Then
        Get #1, , asprin2
        Get #1, , wow2
        If asprin2 = &HFF Then
          txtCode = txtCode & "applymovement APPLYMOVE_PLAYER"
        Else
          txtCode = txtCode & "applymovement 0x" & asprin2
        End If
        If notinmoves(wow2) = True Then
          moves(nummoves) = wow2 - (IIf(wow2 > &H8000000, &H8000000, 0))
          nummoves = nummoves + 1
        End If
        txtCode = txtCode & " 0x" & Hex(wow2)
      ElseIf wite = &H5C Then
        Get #1, , wite2
        Get #1, , asprin2
        Get #1, , asprin3
        Get #1, , wow2
        Get #1, , wow3
        txtCode = txtCode & "trainerbattle 0x" & Hex(wite2) & " 0x" & Hex(asprin2) & " 0x" & Hex(asprin3) & " 0x" & Hex(wow2) & " 0x" & Hex(wow3)
        If notinstrings(wow2) = True Then
          strings(numstrings) = wow2 - (IIf(wow2 > &H8000000, &H8000000, 0))
          numstrings = numstrings + 1
        End If
        If notinstrings(wow3) = True Then
          strings(numstrings) = wow3 - (IIf(wow3 > &H8000000, &H8000000, 0))
          numstrings = numstrings + 1
        End If
        If wite2 = 1 Then
          Get #1, , wow4
          txtCode = txtCode & " 0x" & Hex(wow4)
          If notinsnips(wow4 - IIf(wow4 > &H8000000, &H8000000, 0) + 1) = True Then
            snips(numsnips) = wow4 - IIf(wow4 > &H8000000, &H8000000, 0) + 1
            numsnips = numsnips + 1
          End If
        End If
        If wite2 = 4 Or wite2 = 7 Then
          Get #1, , wow4
          txtCode = txtCode & " 0x" & Hex(wow4)
          If notinstrings(wow4) = True Then
            strings(numstrings) = wow4 - (IIf(wow4 > &H8000000, &H8000000, 0))
            numstrings = numstrings + 1
          End If
        End If
  ...
  If nummoves > 0 Then
    'ns = ns & vbCrLf & "Movements:"
    txtCode = txtCode & vbCrLf & "'-----------"
    txtCode = txtCode & vbCrLf & "' Movements"
    txtCode = txtCode & vbCrLf & "'-----------" & vbCrLf
    
    For Ryan = 0 To nummoves - 1
      ns = ""
      wow = moves(Ryan) - (IIf(moves(Ryan) > &H8000000, &H8000000, 0))
      Do
        If wow = 0 Then Exit Do
        Get #1, wow + 1, wite
        ns = ns & "#raw " & Hex(wite) & " '" & movelabels(wite) & vbCrLf
        If wite = &HFE Then Exit Do
        wow = wow + 1
      Loop
...
Private Sub Form_Load()
  ...
  LastROM = INIRead("EliteMap", "Rubikon", "Last ROM")
...
Private Sub txtCode_KeyDown(KeyCode As Integer, Shift As Integer)
  ...
  oldselstart = txtCode.SelStart
  oldsellength = txtCode.SelLength
  
  SendKeys "{home}", True
  If txtCode.SelLength = 0 Then
    txtCode.SelLength = InStr(1, Mid(txtCode.Text, txtCode.SelStart), " ") - 2
  End If
  ...
  txtCode.SelStart = oldselstart
  txtCode.SelLength = oldsellength
